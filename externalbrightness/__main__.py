...
from . import model
from autobright import colorhugals
...
def main_color(options):
    if options.green is not None:
        for device in options.device:
            ddccontrol.set_maximum_color(
                options.red, options.green, options.blue, device
            )
...
def _parse_args():
    ...
    parser.add_argument(
        "--device", type=int, nargs="+", help="Device numbers for /dev/i2c-X"
    )
    ...
    parser_set.add_argument(
        "--brightness",
        type=int,
        help="Do not query ColorHugALS and use given brightness value",
    )
